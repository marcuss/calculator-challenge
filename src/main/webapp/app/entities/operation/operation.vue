<template>
  <div>
    <h2 id="page-heading" data-cy="OperationHeading">
      <span v-text="$t('calculatorApp.operation.home.title')" id="operation-heading">Operations</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('calculatorApp.operation.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'OperationCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('calculatorApp.operation.home.createLabel')"> Create a new Operation </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && operations && operations.length === 0">
      <span v-text="$t('calculatorApp.operation.home.notFound')">No operations found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="operations">
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('operator')">
              <span v-text="$t('calculatorApp.operation.operator')">Operator</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'operator'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('cost')">
              <span v-text="$t('calculatorApp.operation.cost')">Cost</span>
        ...
        <tbody>
          <tr v-for="operation in operations" :key="operation.id" data-cy="entityTable">
            ...
            <td v-text="$t('calculatorApp.Operator.' + operation.operator)">{{ operation.operator }}</td>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="calculatorApp.operation.delete.question" data-cy="operationDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-operation-heading" v-text="$t('calculatorApp.operation.delete.question', { id: removeId })">
    ...
    <div v-show="operations && operations.length > 0">
