...
describe('UserManagementEdit Component', () => {
  ...
  describe('init', () => {
    it('Should load user', async () => {
      ...
      // THEN
      expect(axiosStub.get.calledWith('api/v1/admin/users/' + 'ABC')).toBeTruthy();
    });
  });
  ...
  describe('initAuthorities', () => {
    it('Should load authorities', async () => {
      ...
      // THEN
      expect(axiosStub.get.calledWith(`api/v1/authorities`)).toBeTruthy();
    });
  });
  ...
  describe('save', () => {
    it('Should call update service on save for existing user', async () => {
      ...
      axiosStub.put.resolves({
        headers: {
          'x-calculatorapp-alert': '',
          'x-calculatorapp-params': '',
        },
      });
      ...
      // THEN
      expect(axiosStub.put.calledWith('api/v1/admin/users', { id: 'ABC', authorities: [] })).toBeTruthy();
    ...
    it('Should call create service on save for new user', async () => {
      ...
      // THEN
      expect(axiosStub.post.calledWith('api/v1/admin/users', { authorities: [] })).toBeTruthy();
      expect(userManagementEdit.isSaving).toEqual(false);
    });
  });
});
